...
const table = createEl('table', main);
...
async function createTable() {
  ...
  currTable.forEach((item, index) => {
    ...
    const row = createEl('tr', table);
    createEl('td', row).textContent = id;
    createEl('td', row).textContent = symbol;
    createEl('td', row).textContent = name;
